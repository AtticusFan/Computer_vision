...
int main(int argc, const char** argv)
{
	...
	Mat rm_Noise;
	medianBlur(Noise_obj, rm_Noise, 3);
	...
	imshow("5.No light division result", rm_Light_Sub);

	Mat Thres_Res_rmlight;
	threshold(rm_Light_Sub, Thres_Res_rmlight, 30, 255, THRESH_BINARY);
	namedWindow("6.Thresholding no light result");
	imshow("6.Thresholding no light result", rm_Light_Sub);
